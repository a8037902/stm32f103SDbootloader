...
u8 SD_ReadDisk(u8*buf,u32 sector,u32 cnt)
{
	...
	if(cnt==1)
	{
		...
		//Debug_Printf(5,"SD_SendCmd=%d\r\n",r1);
		if(r1==0)//指令发送成功
		{
			r1=SD_RecvData(buf,MMC_SD_SECTOR_SIZE);//接收512个字节	   
		}
	}else
	{
...
{
	u8 r1;
	if(SD_Type!=SD_TYPE_V2HC)sector *= MMC_SD_SECTOR_SIZE;//转换为字节地址
	if(cnt==1)
	{
